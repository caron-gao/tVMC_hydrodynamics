#!/usr/bin/env python3
"""
tvmc_mw_shielding_gpu_v8.py
JAX/CUDA implementation of TVMC for microwave-shielded polar molecules

Changes from v7:
  - Added l=4 partial wave: u2 = f0(r)*P0 + f2(r)*P2(cosθ) + f4(r)*P4(cosθ)
  - f4(r) = Σ c4_k * r^4 * exp(-r²/(2ℓ4_k²))
  - New config parameters: n4, ell4_min, ell4_max
  - Parameter layout: [a_rho, a_z, c_rho(n_rho), c_z(n_z), c0(n2), c2(n2), c4(n4)]

Features:
  - Cylindrical trap (omega_rho, omega_z)
  - MW-shielding potential: V_eff = C6/r^6 * [...] + C3/r^3 * (3cos²θ-1)
  - Anisotropic Jastrow: u2 = f0(r) + f2(r)*P2(cosθ) + f4(r)*P4(cosθ)
...
class TVMCConfig:
    ...
    n_z: int = 6
    n2: int = 10
    n4: int = 6           # NEW: number of l=4 basis functions
    ...
    ell2_min: float = 0.01
    ell2_max: float = 0.5
    ell4_min: float = 0.01   # NEW: l=4 length scale range
    ell4_max: float = 0.5    # NEW
    n_walkers: int = 4096
    step_size: float = 0.4
    n_burn: int = 20
    ...
    @property
    def nvar(self) -> int:
        return 2 + self.n_rho + self.n_z + 2 * self.n2 + self.n4
    ...
    def ell4(self) -> Array:
        ...
        return jnp.exp(jnp.linspace(
            jnp.log(self.ell4_min), jnp.log(self.ell4_max), self.n4
        )).astype(Float)
...
def extract_params(p: Array, n_rho: int, n_z: int, n2: int, n4: int) -> CachedParams:
    ...
    return CachedParams(
        ...
        c0=p[idx:idx+n2],
        c2=p[idx+n2:idx+2*n2],
        c4=p[idx+2*n2:idx+2*n2+n4]
    )
...
# ==============================================================================
# NEW: l=4 basis functions: r^4 * exp(-r²/(2ℓ²))
# ==============================================================================

@jit
def g4_reg_vec(r: Array, ell: Array) -> Array:
    """Regular l=4 basis: r⁴ * exp(-r²/(2ℓ²))"""
    r2 = r[..., None] ** 2
    r4 = r2 * r2
    x = r[..., None] / ell
    return r4 * jnp.exp(-0.5 * x * x)


@jit
def g4_reg_dr_vec(r: Array, ell: Array) -> Array:
    """d/dr [r⁴ * exp(-r²/(2ℓ²))] = r³ * exp(...) * (4 - r²/ℓ²)"""
    inv = 1.0 / (ell * ell)
    r_exp = r[..., None]
    r2 = r_exp * r_exp
    r3 = r2 * r_exp
    x = r_exp / ell
    return r3 * jnp.exp(-0.5 * x * x) * (4.0 - r2 * inv)


@jit
def g4_reg_d2r_vec(r: Array, ell: Array) -> Array:
    """d²/dr² [r⁴ * exp(-r²/(2ℓ²))] = r² * exp(...) * (12 - 9r²/ℓ² + r⁴/ℓ⁴)"""
    inv = 1.0 / (ell * ell)
    inv2 = inv * inv
    r2 = r[..., None] ** 2
    r4 = r2 * r2
    x = r[..., None] / ell
    return r2 * jnp.exp(-0.5 * x * x) * (12.0 - 9.0 * r2 * inv + r4 * inv2)
...
@jit
def P4(c: Array) -> Array:
    """Legendre polynomial P₄(c) = (35c⁴ - 30c² + 3)/8"""
    c2 = c * c
    return (35.0 * c2 * c2 - 30.0 * c2 + 3.0) / 8.0


@jit
def dP2_dc(c: Array) -> Array:
    """P₂'(c) = 3c"""
    return 3.0 * c


@jit
def dP4_dc(c: Array) -> Array:
    """P₄'(c) = (140c³ - 60c)/8 = 5c(7c² - 3)/2"""
    return 2.5 * c * (7.0 * c * c - 3.0)
...
@jit
def u2_f0_f2_f4_vec(r: Array, c0: Array, c2: Array, c4: Array,
                     ell2: Array, ell4: Array,
                     kappa_cusp: float, eps_cusp: float, rc_cusp: float
                     ) -> tuple[Array, Array, Array]:
    """Compute f0, f2, and f4 values"""
    ...
    # l=4 channel
    g4_vals = g4_reg_vec(r, ell4)
    f4 = jnp.sum(c4 * g4_vals, axis=-1)
    return f0, f2, f4


@jit
def u2_f0_f2_f4_derivs_vec(r: Array, c0: Array, c2: Array, c4: Array,
                             ell2: Array, ell4: Array,
                             kappa_cusp: float, eps_cusp: float, rc_cusp: float
                             ) -> tuple[Array, Array, Array, Array, Array, Array,
                                        Array, Array, Array]:
    """Compute f0, f0', f0'', f2, f2', f2'', f4, f4', f4''"""
    # f0 (l=0): cusp + Gaussian RBF
    f0 = cusp_u_vec(r, kappa_cusp, eps_cusp, rc_cusp)
    ...
    # f4 (l=4): r⁴ * Gaussian RBF
    g4_vals = g4_reg_vec(r, ell4)
    g4_dr = g4_reg_dr_vec(r, ell4)
    g4_d2r = g4_reg_d2r_vec(r, ell4)
    f4 = jnp.sum(c4 * g4_vals, axis=-1)
    f4p = jnp.sum(c4 * g4_dr, axis=-1)
    f4pp = jnp.sum(c4 * g4_d2r, axis=-1)
    return f0, f0p, f0pp, f2, f2p, f2pp, f4, f4p, f4pp


@jit
def u2_val_vec(r: Array, cos_theta: Array,
               c0: Array, c2: Array, c4: Array,
               ell2: Array, ell4: Array,
               kappa_cusp: float, eps_cusp: float, rc_cusp: float) -> Array:
    """Evaluate u2 = f0(r) + f2(r)*P2(cosθ) + f4(r)*P4(cosθ)"""
    f0, f2, f4 = u2_f0_f2_f4_vec(r, c0, c2, c4, ell2, ell4,
                                   kappa_cusp, eps_cusp, rc_cusp)
    p2 = P2(cos_theta)
    p4 = P4(cos_theta)
    return f0 + f2 * p2 + f4 * p4
...
def compute_log_psi_optimized(pos: Array, omega_ref_rho: float, omega_ref_z: float,
    ...
    def particle_interaction(r_i, all_pos):
        ...
        val = u2_val_vec(r, cos_theta, c0, c2, c4, ell2, ell4,
                         kappa_cusp, eps_cusp, rc_cusp)
...
def metropolis_step_single_particle(pos: Array, i: int, N: int,
    ...
    u2_old = u2_val_vec(r_old, cos_theta_old, c0, c2, c4, ell2, ell4,
                        kappa_cusp, eps_cusp, rc_cusp)
    u2_new = u2_val_vec(r_new, cos_theta_new, c0, c2, c4, ell2, ell4,
                        kappa_cusp, eps_cusp, rc_cusp)
    ...
    def single_walker_sweep(pos, disp, u):
        def body_fn(i, p):
            return metropolis_step_single_particle(
                ...
                c_rho, c_z, ell_rho, ell_z, c0, c2, c4, ell2, ell4,
...
def compute_observables_single_optimized(pos: Array, N: int, n_rho: int, n_z: int, n2: int, n4: int,
    ...
    def particle_derivatives(r_i, all_pos):
        ...
        dz_val = diff[:, 2]
        cos_theta = jnp.clip(dz_val * inv_r, -1.0, 1.0)
        p2 = P2(cos_theta)
        p4 = P4(cos_theta)

        f0, f0p, f0pp, f2, f2p, f2pp, f4, f4p, f4pp = u2_f0_f2_f4_derivs_vec(
            r, c0, c2, c4, ell2, ell4, kappa_cusp, eps_cusp, rc_cusp
        )

        s2 = r2 + s_eps * s_eps  # regularized r²
        dz2 = dz_val * dz_val

        # Gradient of u2 = f0 + f2*P2 + f4*P4
        # For each Pl channel:
        #   ∂(fl*Pl)/∂xi = xi * [fl'*Pl/r - fl*dz*Pl'(cosθ)/(r³)]   for i=x,y
        #   ∂(fl*Pl)/∂z  = above + fl*Pl'(cosθ)/r                     extra for z
        # Using regularized s2 instead of r² to avoid singularities

        # P2 contribution (same as v7)
        dp2 = dP2_dc(cos_theta)  # = 3*cosθ
        f2_ang_r = f2 * dz_val * dp2 / (s2 * r)  # f2 * dz * P2'/(r³) using s2
        f2_ang_z = f2 * dp2 / r                    # f2 * P2'/r

        # P4 contribution (NEW)
        dp4 = dP4_dc(cos_theta)  # = 5*cosθ*(7cos²θ - 3)/2
        f4_ang_r = f4 * dz_val * dp4 / (s2 * r)  # f4 * dz * P4'/(r³)
        f4_ang_z = f4 * dp4 / r                    # f4 * P4'/r

        g_r = (f0p + f2p * p2 + f4p * p4) * inv_r - f2_ang_r - f4_ang_r
        g_zadd = f2_ang_z + f4_ang_z

        grad_x = diff[:, 0] * g_r
        grad_y = diff[:, 1] * g_r
        grad_z = diff[:, 2] * g_r + g_zadd

        # Laplacian: ∇²[fl*Pl] = [fl'' + 2fl'/r - l(l+1)fl/r²]*Pl
        # l=0: l(l+1)=0, l=2: l(l+1)=6, l=4: l(l+1)=20
        lap_val = (f0pp + 2.0 * f0p * inv_r
                   + (f2pp + 2.0 * f2p * inv_r - 6.0 * f2 / s2) * p2
                   + (f4pp + 2.0 * f4p * inv_r - 20.0 * f4 / s2) * p4)
    ...
    def particle_ok_two_body(r_i, all_pos):
        ...
        g4_vals = g4_reg_vec(r, ell4)  # (N, n4)

        ok_c0 = jnp.sum(jnp.where(mask[:, None], g0_vals, 0.0), axis=0)
        ok_c2 = jnp.sum(jnp.where(mask[:, None], g2_vals * p2[:, None], 0.0), axis=0)
        ok_c4 = jnp.sum(jnp.where(mask[:, None], g4_vals * p4[:, None], 0.0), axis=0)

        return ok_c0, ok_c2, ok_c4

    ok_c0_per_particle, ok_c2_per_particle, ok_c4_per_particle = vmap(
        particle_ok_two_body, in_axes=(0, None)
    )(pos, pos)

    O = O.at[idx_c0:idx_c0+n2].set(0.5 * jnp.sum(ok_c0_per_particle, axis=0))
    O = O.at[idx_c2:idx_c2+n2].set(0.5 * jnp.sum(ok_c2_per_particle, axis=0))
    O = O.at[idx_c4:idx_c4+n4].set(0.5 * jnp.sum(ok_c4_per_particle, axis=0))

    return O, E_loc, jnp.mean(rho2), jnp.mean(z2)


@partial(jit, static_argnums=(1, 2, 3, 4, 5))
def compute_observables_batch(X: Array, N: int, n_rho: int, n_z: int, n2: int, n4: int,
                               omega_H_rho: float, omega_H_z: float,
                               omega_ref_rho: float, omega_ref_z: float,
                               C3: float, C6: float, w1_w2: float, w0_w2: float, eps_r: float,
                               a_rho: Complex, a_z: Complex,
    ...
    return vmap(lambda pos: compute_observables_single_optimized(
        pos, N, n_rho, n_z, n2, n4, omega_H_rho, omega_H_z, omega_ref_rho, omega_ref_z,
        C3, C6, w1_w2, w0_w2, eps_r, a_rho, a_z, c_rho, c_z, ell_rho, ell_z,
        c0, c2, c4, ell2, ell4, kappa_cusp, eps_cusp, rc_cusp, s_eps
    ))(X)
...
def run_mcmc_measurements_optimized(X: Array, key: KeyArray,
                                     ...
                                     n_rho: int, n_z: int, n2: int, n4: int,
                                     step_size: float,
                                     omega_H_rho: float, omega_H_z: float,
                                     omega_ref_rho: float, omega_ref_z: float,
                                     C3: float, C6: float, w1_w2: float, w0_w2: float, eps_r: float,
    ...
    def measurement_body(carry, inputs):
        ...
        O, E, R, Z = compute_observables_batch(
            X, N, n_rho, n_z, n2, n4, omega_H_rho, omega_H_z, omega_ref_rho, omega_ref_z,
            C3, C6, w1_w2, w0_w2, eps_r, a_rho, a_z, c_rho, c_z, ell_rho, ell_z,
            c0, c2, c4, ell2, ell4, kappa_cusp, eps_cusp, rc_cusp, s_eps
        )
...
def run_mcmc_and_estimate(X: Array, key: KeyArray, cfg: TVMCConfig,
    ...
    params = extract_params(p, cfg.n_rho, cfg.n_z, cfg.n2, cfg.n4)
    ...
    X, key = run_mcmc_burn_optimized(
        ...
        params.c0, params.c2, params.c4, cfg.ell2, cfg.ell4,
        cfg.kappa_cusp, cfg.eps_cusp, cfg.rc_cusp, cfg.s_eps
    )

    X, key, sO, sE, sR, sZ, sOO, sOE = run_mcmc_measurements_optimized(
        X, key, cfg.n_meas, cfg.thin, cfg.n_walkers, cfg.N,
        cfg.n_rho, cfg.n_z, cfg.n2, cfg.n4, cfg.step_size,
...
def main():
    parser = argparse.ArgumentParser(description="JAX/CUDA TVMC Simulation (v8: with l=4)")
    parser.add_argument("config", nargs="?", default="config.json",
                        help="Path to JSON configuration file (default: config.json)")
    args = parser.parse_args()

    print(f"\n{'='*70}")
    print("JAX/CUDA TVMC v8: MW-Shielding with l=0,2,4 Jastrow")
    print(f"{'='*70}")
    ...
    # NEW: read n4 and ell4 from config, with defaults
    n4 = bas_c.get('n4', 6)
    ell4_min = bas_c.get('ell4_min', bas_c['ell2_min'])
    ell4_max = bas_c.get('ell4_max', bas_c['ell2_max'])
    ...
    cfg = TVMCConfig(
        ...
        ell4_min=ell4_min, ell4_max=ell4_max,
    ...
    print(f"  nvar = {cfg.nvar} (2 + {cfg.n_rho} + {cfg.n_z} + 2×{cfg.n2} + {cfg.n4})")
    print(f"  Partial waves: l=0 ({cfg.n2} basis), l=2 ({cfg.n2} basis), l=4 ({cfg.n4} basis)")
    print(f"  ell2 = [{cfg.ell2_min}, {cfg.ell2_max}], ell4 = [{cfg.ell4_min}, {cfg.ell4_max}]")
    ...
    for it in range(n_imag_steps):
        ...
        if it % 50 == 0 or it == n_imag_steps - 1:
            ...
            # Print l=4 parameter magnitudes for monitoring
            idx_c4 = 2 + cfg.n_rho + cfg.n_z + 2 * cfg.n2
            c4_rms = float(jnp.sqrt(jnp.mean(jnp.abs(state.p[idx_c4:idx_c4+cfg.n4])**2)))
    ...
    # Report l=4 contribution
    idx_c4 = 2 + cfg.n_rho + cfg.n_z + 2 * cfg.n2
    c4_gs = params_gs[idx_c4:idx_c4+cfg.n4]
    c0_gs = params_gs[2+cfg.n_rho+cfg.n_z:2+cfg.n_rho+cfg.n_z+cfg.n2]
    c2_gs = params_gs[2+cfg.n_rho+cfg.n_z+cfg.n2:2+cfg.n_rho+cfg.n_z+2*cfg.n2]

    print(f"\nGround state: E_gs = {E_gs:.6f}, aspect = {aspect_gs:.3f}")
    print(f"  |c0|_rms = {np.sqrt(np.mean(np.abs(c0_gs)**2)):.6f}")
    print(f"  |c2|_rms = {np.sqrt(np.mean(np.abs(c2_gs)**2)):.6f}")
    print(f"  |c4|_rms = {np.sqrt(np.mean(np.abs(c4_gs)**2)):.6f}")
    print(f"  |c4/c0|  = {np.sqrt(np.mean(np.abs(c4_gs)**2)) / max(np.sqrt(np.mean(np.abs(c0_gs)**2)), 1e-12):.4f}")
    ...
    c4_final = final_params[idx_c4:idx_c4+cfg.n4]
    c0_final = final_params[2+cfg.n_rho+cfg.n_z:2+cfg.n_rho+cfg.n_z+cfg.n2]
    c2_final = final_params[2+cfg.n_rho+cfg.n_z+cfg.n2:2+cfg.n_rho+cfg.n_z+2*cfg.n2]
    ...
    print(f"\nl=4 channel analysis:")
    print(f"  Ground state: |c4|_rms = {np.sqrt(np.mean(np.abs(c4_gs)**2)):.6f}")
    print(f"  Final state:  |c4|_rms = {np.sqrt(np.mean(np.abs(c4_final)**2)):.6f}")
    print(f"  |c4/c0| (GS):    {np.sqrt(np.mean(np.abs(c4_gs)**2)) / max(np.sqrt(np.mean(np.abs(c0_gs)**2)), 1e-12):.4f}")
    print(f"  |c4/c0| (final): {np.sqrt(np.mean(np.abs(c4_final)**2)) / max(np.sqrt(np.mean(np.abs(c0_final)**2)), 1e-12):.4f}")
    print(f"  c4 coefficients (GS):    {np.array2string(c4_gs.real, precision=6)}")
    print(f"  c4 coefficients (final): {np.array2string(c4_final.real, precision=6)}")
    ...
    np.savez(results_path,
             ...
             n_rho=cfg.n_rho, n_z=cfg.n_z, n2=cfg.n2, n4=cfg.n4,
             ell2_min=cfg.ell2_min, ell2_max=cfg.ell2_max,
             ell4_min=cfg.ell4_min, ell4_max=cfg.ell4_max,
